#' Get the number of herd (i.e. populations) from GPS data
#'
#' Function to choose the optimal number of herds (i.e. populations),
#' based on GPS data of individuals using NbClust function from the NbClust package.
#' This function provides 30 indices to determine the optimal number of clusters to be used.
#' See ?NbClust for more information.
#'
#' @param df a dataframe containing x and y coordinates and the ID_year
#' (Identifiant of individuals for each year of survey)
#' @param kmin minimum number of herds
#' @param kmax maximum number of herds
#' @param method the cluster analysis method to be used among: "ward.D", "ward.D2",
#' "single", "complete", "average", "mcquitty", "median", "centroid", "kmeans".
#' Default is "kmeans".
#' @param index the index to be calculated. This should be one of : "kl", "ch", "hartigan",
#' "ccc", "scott", "marriot", "trcovw", "tracew", "friedman", "rubin", "cindex", "db",
#' "silhouette", "duda", "pseudot2", "beale", "ratkowsky", "ball", "ptbiserial", "gap",
...
#'
#' @return The function returns a plot of the Number of indices proposing the given optimal number
#' of herds, the optimal number of clusters proposed by each index as well as the value of each
#' index and the Optimal Cluster assignment based on the consensus on the optimal number of cluster.
#'
#' @export
...
#'
#
getKnumber <- function(df, kmin, kmax, method =  "kmeans",
                       index = "all"){
...
myNbClust <-function(data = NULL, diss=NULL, distance ="euclidean", min.nc=2, max.nc=15, method =NULL, index = "all", alphaBeale = 0.1){
...
res <- myNbClust(input, min.nc = kmin, max.nc = kmax, distance = "euclidean", method = method,
               index = index)


barplot(table(res$Best.n[1,])[as.numeric(names(table(res$Best.n[1,])))>=kmin],
        xlab="Optimal number of Clusters", ylab="Frequency among the indices",
        main = "Number of indices proposing \n the given number of Clusters")
