...
downloadWeather <- function (station, dir = tempdir(check = TRUE)) 
{
  ...
  download.file(paste0(url, station, ".dly"), destfile = paste0(dir, 
                                                                "/raw_", station, ".txt"))
  file.name <- paste0(dir, "/raw_", station, ".txt")
  keepcols <- c(2:4, seq(5, 65, 2))
  mywidths <- c(11, 4, 2, 4, rep(c(5, 3), 31))
  mytemp <- read.fwf(file = file.name, widths = mywidths, header = FALSE, 
                     sep = "\t", na.strings = "-999.9")[keepcols]
  ...
  if (nrow(prcp) > 0) {
    ...
    process.prcp <- data.frame(Station = mytemp_sort$station[1], 
                               ...
                               Day = 1:31, prcp = (prcp[, 4:34] %>% 
                                                     as.matrix %>% t %>% as.vector)/10)
  }
  if (nrow(min) > 0) {
  ...
  if (exists("process.max") & exists("process.min")) {
    ...
    all <- temp.all %>% mutate(Tavg = rowMeans(temp.all[c("Tmin", 
                                                          "Tmax")], na.rm = TRUE))
  }
    ...
    all <- data.frame(Station = mytemp_sort$station[1], Tavg = NA, 
                      Tmin = NA, Tmax = NA, prcp = NA)
  }
