...
#' the date and time (Time) of each speed value and the difference in hours 
#' between each speed value and the first one
#' and a vector of the animal identifiant (ID).
...
parturition.model <- function (df, int, kcons, PlotIt = FALSE, saveplot = FALSE,
  ...
  for(i in unique(df$ID_Year))  {
    ...
    ## results table
    results.summary.temp <- data.frame(ID_Year = ID_Year, ID=ID, Year = Year, Best.Model=results.data.temp$Best.Model,
