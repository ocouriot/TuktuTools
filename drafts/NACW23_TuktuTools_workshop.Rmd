...
What are the years of monitoring for each individual?
...
When there are a lot of individuals (sometimes hundreds), It may make more sense to sort the individuals not alphabetically, but by the time of release. Here’s an approach:
...
```
On average, the fix rate is ~4 hours (median is 8 hours), the smallest one is 1 minute and the biggest one is 160 hours, or ~ 7 days.
...
The `prepData` function, from the `TuktuTools` package, allows to filter data for a defined period, using a start date and an end date, irrespective of the year. Thus function also allows to set a minimum average amount of fixes (relocations) per day, which exclude tracks for individuals having less fixes per day than this threshold. Finally, it allows to determine a maximum number of successive days having missing data. The two last parameters are optional. 
This function returns a data frame filtered as desired, and indicate the number of individual-year that have been excluded.
...
Looking back at this figure, we see that it is because those two individuals were not monitored during the calving period those years.

## Visualizing data

`TuktuTools` contains a function to visualize animal tracks: `scan_tracks`. It is helpful to see individuals' trajectories, as well as examining the variation of their x and y locations through time.
Note: for using this function, X,Y columns must be capitalized.
