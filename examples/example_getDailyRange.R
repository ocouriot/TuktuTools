...
days <- days %>% getDailyMean %>% st_as_sf(coords=c("x","y"), crs = CRS) %>% 
  mutate(Year = 2015, Date = as.character(as.Date(paste0("2012", substr(Time, 5, 10)))))

# daily LoCoH
dailyranges.locoh <- getDailyRange(days, method = "LoCoH", nn = 5, level = .95) %>%
...
map <- mapview(days, zcol = "Date")
m2 <- mapview(dailyranges.ud, map = map@map, zcol = "Date")
mapview(dailyranges.locoh, map = m2@map, zcol = "Date")
